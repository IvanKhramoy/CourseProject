...
Game::Game()
    ...
    : mWindow(sf::VideoMode::getDesktopMode(), "BalloonTyper", sf::Style::Default, sf::State::Fullscreen)
...
void Game::drawHUD() {
    ...
    auto drawRightAligned = [&](const std::string& str, float y) {
        sf::Text txt(mRM.font("main"), str, 24);
    ...
    // 3. Центральные надписи (Больше не мигают!)
    
    // Функция для стилизации центральных сообщений (белые с голубой обводкой)
    auto drawCenterMessage = [&](const std::string& textStr) {
        // Y-координата: WINDOW_H / 2.f + 100.f (это чуть ниже середины)
        sf::Text msg = makeText(textStr, 42, sf::Color::White, hudW / 2.f, WINDOW_H / 2.f + 100.f);
        msg.setStyle(sf::Text::Style::Bold);
        msg.setOutlineColor(sf::Color(60, 160, 255)); // Красивый голубой цвет
        msg.setOutlineThickness(4.f);                 // Жирная обводка
        mWindow.draw(msg);
    };
...
void Game::adjustViewports(unsigned int width, unsigned int height) {
    if (height == 0) return; // Защита от сворачивания окна
